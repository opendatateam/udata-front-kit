...
const handleRemoveFactor = async (group: string, index: number) => {
  const factorIndex = getFactorIndex(group, index)
  const factor = factors.value[factorIndex]
  ...
  // User confirmed, now delete via API
  if (factor?.id) {
    await elementStore.deleteElement(props.topicId, factor.id)
  }

  // Update local state with the confirmed changes
