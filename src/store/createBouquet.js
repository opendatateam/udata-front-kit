  ...
  return defineStore ('createBouquet', {
    ...
    actions: {
      ...
      async addInformation(bouquet) {
        const response = await client.update(this.data.id, this.serialize(bouquet))
      ...
      async addDatause(datause) {
        const response = await client.update(this.data.id, this.serialize(datause))
      ...
      serialize(payload) {
        ...
        serializedPayload = { name: this.data.name, description: this.data.description, tags: this.data.tags }
